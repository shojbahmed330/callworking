...
const LiveVideoRoomScreen: React.FC<LiveVideoRoomScreenProps> = ({ currentUser, roomId, onGoBack, onSetTtsMessage }) => {
    ...
    // Main Agora & Firebase Lifecycle Effect
    useEffect(() => {
        let isMounted = true;
        ...
        const unsubRoom = firebaseService.listenToRoom(roomId, 'video', (roomDetails: LiveVideoRoom | null) => {
            if(!isMounted) return;
            if (roomDetails) {
                 if(roomDetails.status === 'ended') {
                     onSetTtsMessageRef.current("The video room has ended.");
                     setTimeout(() => onGoBackRef.current(), 2000);
                 }
                setRoom(roomDetails);
            } else {
                 onSetTtsMessageRef.current("The video room no longer exists.");
                onGoBackRef.current();
            }
        });
        ...
        const initializeAndJoin = async () => {
             try {
                ...
                // 1. Join in Firebase (updates participant list)
                ...
                // 3. Get token and join Agora channel
                ...
                // 4. Get local media and publish
                try {
                    const [audioTrack, videoTrack] = await AgoraRTC.createMicrophoneAndCameraTracks();
                    if (!isMounted) { audioTrack.close(); videoTrack.close(); return; }
                    
                    localAudioTrack.current = audioTrack;
                    localVideoTrack.current = videoTrack;
                    setLocalVideoTrackState(videoTrack); // Update state to trigger re-render
                    ...
                    await client.publish([audioTrack, videoTrack]);
                     ...
                     if (isMounted) {
                        if (mediaError.name === 'NotAllowedError' || mediaError.code === 'PERMISSION_DENIED') {
                            onSetTtsMessageRef.current("Camera/Mic permission denied. You've joined as a viewer.");
                        } else {
                            onSetTtsMessageRef.current("Camera/Mic not found. You've joined as a viewer.");
                        }
                        setIsMuted(true);
                        setIsCameraOff(true);
                     }
    ...
    if (isLoading || !room) {
        return <div className="h-full w-full flex items-center justify-center bg-slate-900 text-white">Joining Video Room...</div>;
    }
    ...
    return (
        <div className="h-full w-full flex flex-col bg-slate-900 text-white">
            ...
            <footer className="flex-shrink-0 p-4 bg-black/20 flex justify-center items-center h-24 gap-6">
                 <button onClick={toggleMute} disabled={!localAudioTrack.current} className={`p-4 rounded-full transition-colors ${!localAudioTrack.current ? 'bg-slate-700/50 cursor-not-allowed' : isMuted ? 'bg-rose-600' : 'bg-slate-600 hover:bg-slate-500'}`}>
                    <Icon name={!localAudioTrack.current || isMuted ? 'microphone-slash' : 'mic'} className="w-6 h-6" />
                </button>
                 <button onClick={toggleCamera} disabled={!localVideoTrack.current} className={`p-4 rounded-full transition-colors ${!localVideoTrack.current ? 'bg-slate-700/50 cursor-not-allowed' : isCameraOff ? 'bg-rose-600' : 'bg-slate-600 hover:bg-slate-500'}`}>
                    <Icon name={!localVideoTrack.current || isCameraOff ? 'video-camera-slash' : 'video-camera'} className="w-6 h-6" />
                </button>
