...
export const PostCard: React.FC<PostCardProps> = ({ post, currentUser, isActive, isPlaying, onPlayPause, onReact, onViewPost, onAuthorClick, onStartComment, onSharePost, onAdClick, onDeletePost, onOpenPhotoViewer, groupRole, isGroupAdmin, isPinned, onPinPost, onUnpinPost, onVote }) => {
  ...
  const getPostTypeString = () => {
    ...
    if (post.postType === 'profile_picture_change') return 'updated their profile picture.';
    if (post.postType === 'cover_photo_change') return 'updated their cover photo.';
    return timeAgo;
  }
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-2 mb-4">
          {isPinned && <span className="text-xs font-bold uppercase tracking-wider px-2 py-1 rounded-full bg-lime-500/20 text-lime-300 flex items-center gap-1"><Icon name="pin" className="w-3 h-3"/> Pinned</span>}
